...
/**
 *   	\file       variedade_list.php
 *		\ingroup    safra
 *		\brief      List page for variedade
 */
...
require_once __DIR__.'/class/variedade.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('safra', 'variedade', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Variedade';
	$objectlabel = 'Variedade';
...
$title = $langs->trans("Variedades");
//$help_url = "EN:Module_Variedade|FR:Module_Variedade_FR|ES:Módulo_Variedade";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/safra/variedade_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/safra/variedade_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendVariedadeRef";
$modelmail = "variedade";
$objecttmp = new Variedade($db);
